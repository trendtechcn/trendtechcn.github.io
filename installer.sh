...
install_driver() {
    ...
    re cd "$(mktemp -d)"
    download "https://deb.trendtechcn.com/rtl$_CHIP-dkms.deb"
...
Please see: https://deb.trendtechcn.com/supported-distributions" \
                cat /etc/os-release
        else
            re $pmcmd $list
        fi
...
Please attach this autogenerated log file to the email:
    /tmp/troubleshooting.txt
Some common problems are documented in:
